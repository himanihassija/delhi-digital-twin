...
DELHI_ZONES = [
    ...
    {"name": "Noida Sector 18", "lat": 28.5695, "lng": 77.3218},
    {"name": "Saket",           "lat": 28.5244, "lng": 77.2090},
    {"name": "Janakpuri",       "lat": 28.6282, "lng": 77.0822},
    {"name": "Nehru Place",     "lat": 28.5491, "lng": 77.2536},
    {"name": "Karol Bagh",      "lat": 28.6514, "lng": 77.1907},
    {"name": "Vasant Kunj",     "lat": 28.5273, "lng": 77.1571},
]
...
def count_transports(citizens):
    return Counter(c["current_transport"] for c in citizens)
...
def get_transport_scores(citizen, time_slot="Morning Peak"):
    ...
    # ── Original 4 ────────────────────────────────
        ...
        if citizen["type"] in ["Female Student", "Female Office Worker"]:
    ...
    # ── 8 New policies ────────────────────────────
        ...
        # Late-night accessibility — boosts safety & convenience
        transports["Metro"]["safety"]  = min(10, transports["Metro"]["safety"]  + 2)
        transports["Metro"]["comfort"] = min(10, transports["Metro"]["comfort"] + 1)
        transports["Auto"]["cost"]     = max(1,  transports["Auto"]["cost"]     - 2)  # autos less needed

    elif POLICY == "Airport Express Fare Reduction":
        # Cheaper express = metro more attractive for long commutes
        if citizen["commute_distance_km"] >= 15:
        ...
        if citizen["type"] in ["Female Student"]:
        ...
        # High-carbon users (car/auto) penalised; public transit rewarded
        ...
        # Small random boost — 1-in-365 chance each citizen has birthday today
        if random.random() < (1/365 * 100):   # scaled for demo with 100 citizens
            transports["Metro"]["cost"] = 10
            transports["Bus"]["cost"]   = 10
        ...
        # Seamless transfers — time and comfort improve across all public transit
        transports["Metro"]["time"]    = min(10, transports["Metro"]["time"]    + 2)
        ...
        # Doesn't directly boost public transit — mainly benefits car owners
        # Slight nudge toward walking for short trips (parking freed up near destinations)
        if citizen["commute_distance_km"] <= 5:
...
def run_once(citizens_input, time_slot="Morning Peak"):
    ...
    for c in citizens:
        ...
        if old != new:
            switches += 1
        if old in ["Auto Rickshaw","Auto"] and new == "Metro":
            co2_reduction += 5
        elif old == "Bus" and new == "Metro":
            co2_reduction += 2
        elif old in ["Car","Scooter","Bike"] and new == "Metro":
            co2_reduction += 4
        elif old == "Cab" and new == "Metro":
            co2_reduction += 3
...
def run_with_confidence(citizens_base, time_slot="Morning Peak", n=5):
    ...
    for _ in range(n):
        ...
        private_before = sum(before.get(m,0) for m in ["Car","Auto Rickshaw","Bike","Scooter","Cab"])
        private_after  = sum(after.get(m,0)  for m in ["Car","Auto Rickshaw","Bike","Scooter","Cab"])
...
def compute_equity(citizens_before, citizens_after):
    ...
    for cb, ca in zip(citizens_before, citizens_after):
        ...
        brackets[br]["total"] += 1
        if cb["current_transport"] in public_modes: brackets[br]["before_pub"] += 1
        if ca["current_transport"] in public_modes: brackets[br]["after_pub"]  += 1
    ...
    for br, d in brackets.items():
        ...
        result[br] = {"before_pct": before_pct, "after_pct": after_pct,
                      "gain": round(after_pct - before_pct, 1), "total": d["total"]}
...
def run_time_of_day(citizens_base):
    ...
    for slot in TIME_SLOTS:
        ...
        results[slot] = {
            ...
            "metro_share":     round(after.get("Metro",0) / max(sum(after.values()),1) * 100, 1)
        }
    return results

# =====================================================
# CITIZEN SENTIMENT — covers all 12 policies
# =====================================================

POLICY_SENTIMENT = {
    "Free Metro Rides For Women": {
        "benefits": ["Free metro access for female commuters", "Improved gender equity in transit"],
        "complaints": ["Limited coverage in outer districts", "Peak-hour crowding on metro"]
    },
    ...
    "Reserved Student Coaches": {
        "benefits": ["Safer commute for students", "Higher metro adoption among youth"],
    ...
    "Free Transit Birthdays": {
        "benefits": ["Fun, viral policy — boosts public goodwill", "Encourages first-time metro users"],
        "complaints": ["Difficult to verify without ID checks", "Negligible impact on daily ridership"]
    },
    "Car-Free School Zones": {
        "benefits": ["Safer walking routes for children", "Reduced school-run traffic"],
...
def compute_sentiment(switches, co2, after, before, policy):
    ...
    base = 60
    if switches > 30: base += 20
    elif switches > 15: base += 10
    if co2 > 20: base += 10
    elif co2 > 10: base += 5
...
def generate_map_data(citizens):
    ...
    for i, c in enumerate(citizens):
        zone = DELHI_ZONES[i % len(DELHI_ZONES)]
        t = c["current_transport"]
        if t in ["Auto Rickshaw","Cab","Car","Scooter","Bike"]: display = "Auto"
        elif t == "Walking": display = "Walking"
        elif t == "Bus":     display = "Bus"
        else:                display = "Metro"
        points.append({
            "id": c["id"], "type": c["type"],
            "zone": zone["name"],
...
def create_transport_chart(before, after):
    ...
    ax.legend()
    plt.tight_layout(); plt.savefig("transport_impact.png", dpi=100, transparent=True); plt.close()
...
def create_co2_chart(co2):
    ...
    ax.text(0, co2+0.3, str(co2), ha="center", va="bottom", color=color, fontsize=14, fontweight="bold")
    ax.set_ylabel("Score"); ax.set_title(f"CO2 Impact — {POLICY}", pad=10)
    for s in ["top","right"]: ax.spines[s].set_visible(False)
    for s in ["bottom","left"]: ax.spines[s].set_color("#94a3b8")
    plt.tight_layout(); plt.savefig("co2_impact.png", dpi=100, transparent=True); plt.close()
...
def government_agent_text(r):
    ...
    pm = ["Car","Auto Rickshaw","Bike","Scooter","Cab"]
    priv_b = sum(r["transport_before"].get(m,0) for m in pm)
    priv_a = sum(r["transport_after"].get(m,0)  for m in pm)
    decision = "APPROVE" if increase > 5 else "REVIEW"
    return (f"Recommendation: {decision}\nPolicy: {r['policy']}\n"
            f"Metro Growth: {increase} users\nCongestion Reduction: {priv_b-priv_a} private vehicles shifted\n"
            f"CO2 Score: {r['estimated_co2_reduction']}\n"
            f"Impact: {'effective' if increase > 0 else 'weak'}.")
...
def transport_agent_text(r):
    ...
    return (f"Total Mode Switches: {r['transport_changes']}\n"
            f"Metro demand: +{mg}\nBus demand: {bc:+d}\n"
            f"{'Increase metro frequency during peak hours.' if mg > 20 else 'Monitor ridership and adjust.'}")

def environment_agent_text(r):
    co2 = r["estimated_co2_reduction"]
    level = "High" if co2>50 else "Medium" if co2>20 else "Low"
    return f"Environmental Impact: {level} Benefit\nCO2 Score: {co2}\nShift toward public transport improves air quality."
...
def citizen_agent_text(r):
    ...
    return (f"{r['transport_changes']} citizens changed behavior.\n"
            ...
            f"Benefits: {', '.join(s.get('top_benefits',[]))}\n"
            f"Concerns: {', '.join(s.get('top_complaints',[]))}")
...
def business_agent_text(r):
    ...
    return (f"Metro Users After: {ma} (new: {ma-mb})\n"
            f"Footfall near metro corridors: +{(ma-mb)*2} persons/day\n"
            f"Estimated revenue uplift near stations: {round((ma-mb)*0.15,1)}%")

# =====================================================
# MAIN
# =====================================================

with open("enhanced_citizens.json","r") as f:
...
with open("updated_citizens.json","w") as f:
    json.dump(citizens_updated, f, indent=4)
...
print(f"CO2:      {co2}         [{ci_data['co2'][2]}–{ci_data['co2'][3]}]")
print(f"Switches: {switches}        [{ci_data['switches'][2]}–{ci_data['switches'][3]}]")
